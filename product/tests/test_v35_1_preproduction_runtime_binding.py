...
def _event(operation, payload):
    ...
    return {
        "event_type": "V35_1_PROXY_RUNTIME_COMMAND_RECEIVED",
...
def test_v35_1_set_control_method_updates_worker_state_safely():
...
def test_v35_1_parameter_update_updates_user_config_runtime_parameters():
    resources = _resources()
...
def test_v35_1_state_read_and_snapshot_are_available():
    resources = _resources()
...
def test_v35_1_thread_management_handler_records_runtime_result_without_duplicate_apply():
    resources = _resources()
...
def test_v35_1_bridge_reply_uses_central_runtime_binding():
    resources = _resources()
    runtime_bundle = {
        "settings": {
            "proxy_worker_bridge": {
    ...
    assert reply["bridge"] == "v35_1_preproduction_final_runtime"
    assert reply["result"]["status"] == "applied_to_validated_runtime_state"
    assert resources["worker_state"]["data"]["RUNTIME_CONTROL_METHOD"] == "auto"


def test_v35_1_runtime_state_has_proxy_runtime_command_state_metadata():
    ...
    assert data["binding_version"] == "v35_1_preproduction_final_runtime"
